# Lobster Mini Harness 与 DeepSeek Harness 对照

本文件是 30 天路线 Day 18 的产物：对照 Lobster 自研的 Mini Harness
（`services/api/app/agent/`）与 DeepSeek Harness（`C:\Users\tongd\Desktop\deepseek`
monorepo）的核心模块，说明两份实现的异同，以及 Lobster 未来以插件方式接入
DeepSeek Harness 的路径。

## 1. 总览

| 维度 | Lobster Mini Harness | DeepSeek Harness |
|---|---|---|
...
- **Lobster**：`app/agent/providers/deepseek.py` 实现 `complete(messages, tools) -> AssistantTurn`，
...
**不同点**：DeepSeek 把装配/错误/消息构造拆得更细，Lobster 为了教学收敛成一个 `complete`。

### 2.2 工具注册与执行

- **Lobster**：`ToolRegistry.register(name, description, input_model, handler, timeout, permission)`，
...
**不同点**：Lobster 的轨迹在 `AgentEvent` 数据里随 SSE 流式返回；DeepSeek 把每一次派发
写成 session 事件（append-only log），UI 和持久化都读同一份 log。

### 2.3 Agent Loop

- **Lobster**（`loop.py`）：`stream()` 构建 `[system, history, user]` → 循环 `model → tool → model`
...
**不同点**：Lobster 是显式 `for` 循环，控制流直白；DeepSeek 是事件驱动的状态机，能处理
并发唤醒、取消和维护任务，复杂度更高但更健壮。

### 2.4 Session / 记忆

- **Lobster**：`InMemoryConversationStore` / `PostgresConversationStore` 存消息；`compaction.py`
...
**不同点**：Lobster 把摘要游标存在数据库列里；DeepSeek 把压缩也作为 session 事件的一部分。

### 2.5 权限与人工确认

- **Lobster**：`ToolPermission = read | write | simulate | trade | prohibited`；`simulate`/`trade`
...
- **Lobster**：`app/persistence/` 用 SQLAlchemy ORM + Alembic 迁移，表结构显式
...
**关键差异**：Lobster 是“关系表 + 显式状态”；DeepSeek 是“事件溯源 + 派生视图”。
事件溯源更适合重放与审计，Lobster 的显式表更适合教学和直接查询。

## 3. 关键架构差异小结

1. **控制流模型**：显式 `for` 循环（Lobster） vs 事件驱动状态机（DeepSeek）。
2. **真相来源**：关系表字段（Lobster） vs append-only 事件日志（DeepSeek）。
3. **工具轨迹**：随 SSE 流内联返回（Lobster） vs 写回 session 事件（DeepSeek）。
4. **语言边界**：Python 服务端（Lobster） vs TypeScript 全栈 monorepo（DeepSeek）。
5. **可扩展性**：Lobster 用 `register_*_tools` 函数集中装配；DeepSeek 用 scope /
   extensions / skill / plugins 分层注册，插件边界更清晰。

## 4. 以插件方式接入 DeepSeek Harness 的路径

roadmap 约定“正式 Lobster 以插件方式运行，不直接魔改 Harness 核心”。基于以上对照，
接入路径是：

1. **数据工具插件**：把 Lobster 的 Binance / 持仓 / 风险 / 回测 / 交易工具，用
   `@deepseek-ai/dsh-tools` 的 schema 重新包装成 DeepSeek 工具，复用现有确定性逻辑。
2. **配置与 Skill**：用 `skill` / `extensions` 包声明 Lobster 的 system prompt、
   权限分级和工具白名单，不侵入 `agent-loop` 核心。
3. **持久化桥接**：Lobster 的 PostgreSQL 表继续保留作为“业务数据”，DeepSeek 的
   session 事件作为“对话轨迹”，两者通过 session_id 关联，而不是强行二选一。
4. **Web 入口**：Lobster 现有的 React + SSE 界面可以保留为独立前端，DeepSeek Harness
   通过 `apps/web` / `host` 提供宿主，Lobster 作为其中的一个 agent / 插件形态运行。
